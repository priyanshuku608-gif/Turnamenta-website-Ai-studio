...
export const Header: React.FC<HeaderProps> = ({
  ...
  const displayName = userProfile?.displayName || currentUser?.displayName || 'admin';
    ...
    <header className="sticky top-0 z-40 bg-[#0F172A]/95 backdrop-blur-md border-b border-slate-800/80 px-4 py-3 flex items-center justify-between shadow-lg">
      ...
      <div className="flex items-center gap-3">
        {showBack ? (
          <button
            onClick={onBack}
            className="w-9 h-9 rounded-full bg-slate-800/80 border border-slate-700/60 flex items-center justify-center text-slate-200 active:scale-95 transition-transform"
            aria-label="Go back"
          >
        ...
        <div>
          {title ? (
            <h1 className="font-bold text-lg text-white tracking-wide truncate max-w-[140px] sm:max-w-[200px]">
              {title}
            </h1>
          ) : (
            <div>
      ...
      <div className="flex items-center gap-2.5">
        ...
        <button
          ...
          className="relative w-9 h-9 rounded-full bg-slate-800/60 border border-slate-700/50 flex items-center justify-center text-slate-300 hover:text-white active:scale-95 transition"
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#B6FF3C] text-black font-bold text-sm tracking-tight hover:brightness-105 active:scale-95 transition shadow-[0_0_12px_rgba(182,255,60,0.3)]"
          aria-label="Open Wallet"
        >
          <WalletIcon className="w-4 h-4 fill-black text-black stroke-[2.5]" />
